...
void main() {
  group('.packages', () {
    test('valid', () {
      var packagesFile =
          '# Generated by pub yadda yadda\n'
          'foo:file:///foo/lib/\n'
          'bar:/bar/lib/\n'
          'baz:lib/\n';
      var result = packages.parse(
        utf8.encode(packagesFile),
        Uri.parse('file:///tmp/file.dart'),
        throwError,
      );
      expect(result.version, 1);
      expect({for (var p in result.packages) p.name}, {'foo', 'bar', 'baz'});
      expect(
        result.resolve(pkg('foo', 'foo.dart')),
        Uri.parse('file:///foo/lib/foo.dart'),
      );
      expect(
        result.resolve(pkg('bar', 'bar.dart')),
        Uri.parse('file:///bar/lib/bar.dart'),
      );
      expect(
        result.resolve(pkg('baz', 'baz.dart')),
