...
Future<void> main(List<String> args) async {
  final parser = ArgParser()
    ..addOption(
      'max-packages',
      abbr: 'm',
      help: 'Maximum number of packages to fetch',
    )
  ...
  final namesResponse = await http.get(
    Uri.parse('https://pub.dev/api/package-names'),
  );
  ...
  await Future.wait(
    packagesToProcess.map(
      (package) async => pool.withResource(() async {
        final outFile = File('${outDir.path}/$package.jsonl.gz');
        if (outFile.existsSync()) {
          completed++;
          print(
            '[$completed/${packagesToProcess.length}] '
