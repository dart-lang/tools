...
class BenchmarkReport {
  ...
  static String formatTable(
    ...
    for (final result in results) {
      ...
      buffer.writeln(
        ...
        '${(result.cv * 100).toStringAsFixed(1).padLeft(6)} | '
        '${ratioStr.padLeft(8)}',
      );
    }
  ...
  /// Returns a reliability warning if a result has high variance or is
  /// unstable.
  static String? getReliabilityWarning(BenchmarkResult result) {
    if (!result.isStable) {
      return 'Warning: ${result.name} failed to reach mathematical steady '
          'state during the warmup budget. Results may be unreliable.';
    }
    if (result.cv > 0.2) {
      return 'Warning: ${result.name} has high variance '
          '(CV: ${(result.cv * 100).toStringAsFixed(1)}%). '
