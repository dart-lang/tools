...
void main() {
  ...
  group('list()', () {
    ...
    test(
      'returns empty list for non-existent case-sensitive directories',
      () async {
        expect(
          await Glob('non/existent/**', caseSensitive: true).list().toList(),
          <Never>[],
        );
      },
    );
    ...
    test(
      'returns empty list for non-existent case-insensitive directories',
      () async {
        expect(
  ...
  group('listSync()', () {
    ...
    test('returns empty list for non-existent case-sensitive directories', () {
      expect(
        Glob('non/existent/**', caseSensitive: true).listSync(),
        <Never>[],
      );
    });
    ...
    test(
      ...
      () {
        expect(
          Glob('non/existent/**', caseSensitive: false).listSync(),
          <Never>[],
        );
      },
    );
  ...
  group('when case-sensitive', () {
    ...
    test('lists ranges case-sensitively', () {
      expect(
        Glob('foo/[BX][A-Z]z/qux', caseSensitive: true).listSync(),
        <Never>[],
      );
    });
    ...
    test('options preserve case-sensitivity', () {
      expect(
        Glob('foo/{BAZ,ZAP}/qux', caseSensitive: true).listSync(),
        <Never>[],
      );
    });
  });
  ...
  syncAndAsync((ListFn list) {
    ...
    group('range', () {
      ...
      test("doesn't match outside its range", () async {
        expect(
          await list('foo/ba[a-x]'),
          unorderedEquals([p.join('foo', 'bar')]),
        );
      });

      test("doesn't match outside its specific list", () async {
        expect(
          await list('foo/ba[rx]'),
          unorderedEquals([p.join('foo', 'bar')]),
        );
      });
    });
    ...
    test(
      "the same file shouldn't be non-recursively listed multiple times",
      () async {
        await d.dir('multi', [
        ...
        expect(
          await list('multi/{start-*/f*,*-end/*e}'),
    ...
    test(
      "the same file shouldn't be recursively listed multiple times",
      () async {
        await d.dir('multi', [
          d.dir('a', [
            d.dir('b', [
    ...
    group('with symlinks', () {
      ...
      test("doesn't follow symlinks with followLinks: false", () async {
        expect(
          await list('dir/**', followLinks: false),
    ...
    test('lists an absolute glob', () async {
      var pattern = separatorToForwardSlash(
        p.absolute(p.join(d.sandbox, 'foo/baz/**')),
      );
    ...
    group('when case-insensitive', () {
      test('lists literals case-insensitively', () async {
        expect(
          await list('foo/baz/qux', caseSensitive: false),
          equals([p.join('foo', 'baz', 'qux')]),
        );
        expect(
          await list('foo/BAZ/qux', caseSensitive: false),
      ...
      test('lists ranges case-insensitively', () async {
        expect(
          await list('foo/[bx][a-z]z/qux', caseSensitive: false),
          equals([p.join('foo', 'baz', 'qux')]),
        );
        expect(
          await list('foo/[BX][A-Z]z/qux', caseSensitive: false),
      ...
      test('options preserve case-insensitivity', () async {
        expect(
          await list('foo/{bar,baz}/qux', caseSensitive: false),
          equals([p.join('foo', 'baz', 'qux')]),
        );
        expect(
          await list('foo/{BAR,BAZ}/qux', caseSensitive: false),
...
typedef ListFn = FutureOr<List<String>> Function(
  String glob, {
  bool recursive,
  bool followLinks,
  bool? caseSensitive,
